...
export default function EditarProyectoPage() {
  ...
  // Portada: puede ser la existente (string base64) o un archivo nuevo
  const [existingCover, setExistingCover] = useState<string>("");
  const [newCoverFile, setNewCoverFile] = useState<File | null>(null);
  const [newCoverPreview, setNewCoverPreview] = useState<string>("");

  // Carrusel: imágenes existentes a conservar + nuevas a agregar
  ...
  function handleNewCover(e: React.ChangeEvent<HTMLInputElement>) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const coverImage = newCoverFile
        ? await compressImage(newCoverFile, 1200, 0.8)
        : existingCover;

      const newGalleryBase64 = await Promise.all(
        newGalleryFiles.map((f) => compressImage(f, 900, 0.75))
      );
      const images = [...existingGallery, ...newGalleryBase64];
  ...
  const coverSrc = newCoverPreview || existingCover;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <header className="border-b border-slate-200 bg-white px-6 py-4 sm:px-10">
        <div className="mx-auto flex max-w-3xl items-center gap-4">
          ...
          <h1 className="text-xl font-semibold text-slate-950">Editar Proyecto</h1>
        </div>
      </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="space-y-3">
            ...
            {coverSrc ? (
              <div className="relative group overflow-hidden rounded-2xl bg-slate-100 aspect-video max-h-64">
                ...
                <div className="absolute inset-0 flex items-end gap-2 p-3 opacity-0 group-hover:opacity-100 transition">
                  <button
                    type="button"
                    onClick={() => coverInputRef.current?.click()}
                    className="flex-1 rounded-full bg-white/90 px-3 py-1.5 text-xs font-semibold text-slate-800 hover:bg-white"
                  >
                    Cambiar
                  </button>
                  <button
                    type="button"
                    onClick={removeCover}
                    className="flex-1 rounded-full bg-red-600/90 px-3 py-1.5 text-xs font-semibold text-white hover:bg-red-600"
                  >
                    Quitar
                  </button>
                </div>
            ...
            {(existingGallery.length > 0 || newGalleryPreviews.length > 0) && (
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {existingGallery.map((src, i) => (
                  <div key={`existing-${i}`} className="relative group overflow-hidden rounded-2xl bg-slate-100 aspect-video">
                ...
                {newGalleryPreviews.map((src, i) => (
                  <div key={`new-${i}`} className="relative group overflow-hidden rounded-2xl bg-slate-100 aspect-video ring-2 ring-blue-400">
                    <Image src={src} alt={`Nueva ${i + 1}`} fill className="object-cover" />
                    <span className="absolute left-2 top-2 rounded-full bg-blue-500 px-2 py-0.5 text-[10px] font-semibold text-white">Nueva</span>
                    <button
                      type="button"
          ...
          {loading && (
            <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
              Comprimiendo imágenes y guardando cambios...
            </div>
          )}
